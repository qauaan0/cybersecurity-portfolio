...
# Lessons Learned 
1. Context is critical in investigations — each finding builds on the previous one. 
Understanding how questions and events connect to each other is what separates 
isolated observations from a coherent attack narrative.

2. Chronological ordering reveals the full picture — sorting events by timestamp 
rather than investigation order exposed that the system was already compromised 
before the main attack chain began, which would have been missed otherwise.

3. Kibana as an investigative tool — gained hands-on experience navigating Kibana's 
Discover view, alert dashboard, and process tree analyzer to pivot across multiple 
